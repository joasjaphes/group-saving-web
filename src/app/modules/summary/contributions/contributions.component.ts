...
export class ContributionsComponent implements OnInit {
  ...
  constructor(
    ...
    this.contributions$ = this.store.pipe(
      select(paymentSelector.selectDetailedGroupByMember(this.year))
    );
    this.contributionTypes$ = this.store.pipe(
      select(paymentSelector.selectContributionTypeSummary(this.year, 'All'))
    );
  }
  ...
  viewMemberContributions(member) {
    this.currentPayments$ = this.store.pipe(
      select(
        paymentSelector.selectContributionByMemberByYear(this.year, member.id)
      )
    );
    this.viewDetails = true;
  ...
  getData() {
    if (this.currentView === 'members') {
      ...
      this.contributions$ = this.store.pipe(
        select(paymentSelector.selectDetailedGroupByMonth(this.year))
      );
    }
    this.contributionTypes$ = this.store.pipe(
      select(paymentSelector.selectContributionTypeSummary(this.year, 'All'))
  ...
  viewContributions(payment: any) {
    if (payment.type === 'member') {
      this.currentPayments$ = this.store.pipe(
        select(
          paymentSelector.selectContributionByMemberByYear(
            this.year,
            payment.id
          )
        )
      ...
      this.currentPayments$ = this.store.pipe(
        select(
          paymentSelector.selectContributionByMonthByYear(
            this.year,
            payment.key
          )
        )
  ...
  viewByType(contr: { name: string; total: number; id: string }) {
    this.currentPayments$ = this.store.pipe(
      select(
        paymentSelector.selectContributionByTypeByYear(this.year, contr.id)
      )
    );
    this.panelTitle =
      this.year +
      ...
      contr.name +
      ' (' +
      formatNumber(contr.total, 'en-US', '1.0-0') +
      ')';
    this.viewDetails = true;
    this.viewType = 'view';
  }
