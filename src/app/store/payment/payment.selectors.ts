...
export const selectCurrentState = createFeatureSelector<fromReducer.State>(
  fromReducer.paymentsFeatureKey
);
...
export const selectDetailed = createSelector(
  ...
  (allItems, contributionTypes, fineTypes, loanTypes, loans, members) => {
    return allItems.map((item) => {
      const contrDetails = [];
      const paymentItems = [];
      const contributionsDetails = !!item.contributions
        ? Object.keys(item.contributions).map((contrId) => ({
            contr_id: contrId,
            name: contributionTypes[contrId]
              ? contributionTypes[contrId].name
              : '',
      ...
      const fineDetails = !!item.fines
        ...
        : null;
      const loanDetails = !!item.loans
        ? Object.keys(item.loans).map((contrId) => ({
            id:
              loans[contrId] && loanTypes[loans[contrId].loan_used]
                ? loanTypes[loans[contrId].loan_used].id
                : '',
            name:
              loans[contrId] && loanTypes[loans[contrId].loan_used]
                ? loanTypes[loans[contrId].loan_used].name
                : '',
      ...
      if (contributionsDetails) {
        contrDetails.push(
          ...contributionsDetails.map(
            (i) => `${i.name} ${numberWithCommas(i.amount)}`
          )
        );
      ...
      if (loanDetails) {
        contrDetails.push(
          ...loanDetails.map((i) => `${i.name} ${numberWithCommas(i.amount)})`)
        );
...
export const selectDetailedGroupByMember = (year) =>
  createSelector(selectDetailed, fromMember.selectAll, (allItems, members) => {
    const memberData = {};
    for (const member of members) {
      ...
      memberData[member.id].items = Object.keys(
        memberData[member.id].totals
      ).map((i) => memberData[member.id].totals[i]);
    }
    return Object.keys(memberData)
      .map((i) => ({
        ...memberData[i],
        description: memberData[i].items
          .map((k) => `${k.name} ${numberWithCommas(k.amount)}`)
          .join(', '),
      }))
      .filter((i) => !!i.total);
  });

export const selectDetailedGroupBySingleMember = (year, memberId) =>
  createSelector(selectDetailedGroupByMember(year), (items) => {
    return items.filter((i) => i.id === memberId)[0];
  });

export const selectDetailedGroupByMonth = (year) =>
  createSelector(selectDetailed, (allItems) => {
    ...
    for (const month of monthNames) {
      ...
      monthData[key].items = Object.keys(monthData[key].totals).map(
        (i) => monthData[key].totals[i]
      );
    }
    return Object.keys(monthData)
      .map((i) => ({
        ...monthData[i],
        description: monthData[i].items
          .map((k) => `${k.name} ${numberWithCommas(k.amount)}`)
          .join(', '),
      }))
      .filter((i) => !!i.total);
...
export const selectFinesDetailedGroupByMember = (year, fineType) =>
  createSelector(selectDetailed, fromMember.selectAll, (allItems, members) => {
    const memberData = {};
    for (const member of members) {
      memberData[member.id] = {
        ...
        .forEach((item) => {
          if (item.memberId === member.id) {
            for (const contr of item.fineDetails) {
              memberData[member.id].fineKeys =
                fineType === 'All' ? item.fineKeys : [fineType];
              if (fineType === 'All' || fineType === contr.id) {
                if (memberData[member.id].totals[contr.id]) {
                  memberData[member.id].totals[contr.id]['amount'] +=
                    parseFloat(contr.amount);
                } else {
    ...
    const payments = Object.keys(memberData)
      .map((i) => ({
        ...memberData[i],
        description: memberData[i].items
          .map((k) => `${k.name} ${numberWithCommas(k.amount)}`)
...
export const selectFineDetailedGroupByMonth = (year, fineType) =>
  createSelector(selectDetailed, (allItems) => {
    const monthNames = [
      { name: 'Jan ' + year, key: '01' },
      { name: 'Feb ' + year, key: '02' },
    ...
    for (const month of monthNames) {
      ...
      allItems
        ...
        .forEach((item) => {
          if (item.month === month.key && item.year + '' === year + '') {
            for (const contr of item.fineDetails) {
              monthData[month.key].fineKeys =
                fineType === 'All' ? item.fineKeys : [fineType];
              if (fineType === 'All' || fineType === contr.id) {
                if (monthData[month.key].totals[contr.id]) {
                  monthData[month.key].totals[contr.id]['amount'] += parseFloat(
                    contr.amount
                  );
                } else {
      ...
      monthData[month.key].items = Object.keys(monthData[month.key].totals).map(
        (i) => monthData[month.key].totals[i]
      );
    }
    return Object.keys(monthData)
      .map((i) => ({
...
export const selectYearsWithPayment = createSelector(selectAll, (allItems) => {
  const years = [];
...
export const selectContributionTypeSummary = (year, memberId) =>
  createSelector(
    selectDetailed,
          ...
          .forEach((item) => {
            if (item.contributions) {
              if (Object.keys(item.contributions).indexOf(contr.id) !== -1) {
                summary[contr.id].total += parseFloat(
                  item.contributions[contr.id] + ''
                );
              }
            }
          });
...
export const selectFineTypesSummary = (year) =>
  createSelector(
    selectDetailed,
...
export const selectContributionOnlyByMember = (memberId) =>
  createSelector(selectDetailed, (allItems) =>
    allItems.filter(
      (i) => memberId === i.memberId && i.contributionsDetails.length > 0
    )
  );

export const selectContributionByTypeByYear = (year, typeId) =>
  createSelector(selectDetailed, (allItems) =>
    allItems
      .filter((i) => i.year + '' === year + '' || year === 'All')
      .filter(
        (i) => !!i.contributionsDetails.find((k) => k.contr_id === typeId)
      )
      .map((i) => ({
        ...i,
        totalContributions: i.contributionsDetails
          .filter((k) => k.contr_id === typeId)
          .map((k) => k.amount)
          .reduce((j, k) => k + j),
        keys: i.contributionsDetails
          .filter((k) => k.contr_id === typeId)
          .map((k) => k.contr_id),
        description: i.contributionsDetails
          .filter((k) => k.contr_id === typeId)
          .map((k) => `${k.name} ${numberWithCommas(k.amount)}`)
          .join(', '),
      }))
...
export const selectContributionByMonthByYear = (year, month) =>
  createSelector(selectDetailed, (allItems) =>
    allItems
      .filter(
...
export const selectContributionByMemberByYear = (year, memberId) =>
  createSelector(selectDetailed, (allItems) => {
...
export const selectContributionMemberMonthSummary = (
  months: string[],
  contributionTypeId
) =>
  createSelector(selectDetailed, fromMember.selectAll, (allItems, members) => {
    return members.map((member) => {
      ...
      months.forEach((month) => {
        ...
        const memberContr = allItems
          ...
          .filter(
            (i) =>
              !!i.contributionsDetails.find(
                (k) =>
                  k.contr_id === contributionTypeId ||
                  contributionTypeId === 'All'
              )
          )
          .filter(
            (i) => i.year + '' === year + '' && i.month + '' === mon + ''
          );
        memberMonth[key] = 0;
        memberContr.forEach((item) => {
          for (const contr of item.contributionsDetails) {
            if (
              contributionTypeId === 'All' ||
              contr.contr_id === contributionTypeId
            ) {
...
export const selectExpectedCollection = (period: string) =>
  createSelector(
    fromContributionTypes.selectRepeating,
    fromLoan.selectAll,
    ...
    (contributionTypes, loans, members) => {
      ...
      let totalExpected = 0;
      activeLoans.forEach(
        (i) => (totalLoan += parseFloat(i.amount_per_return + ''))
      );
      const contrReturs = contributionTypes
        .map((i) => {
          if (i.is_must && i.is_fixed) {
            ...
            totalExpected +=
              parseFloat(i.minimum_contribution + '') * members.length;
            return {
              name: i.name,
              value: i.minimum_contribution * members.length,
            };
          } else {
            return null;
          }
      ...
      return {
        total: parseInt(totalExpected + '', 10),
        amounts: [
          ...contrReturs,
          { name: 'Loans', value: parseInt(totalLoan + '', 10) },
        ],
      };
    }
  );

export const selectMembersExpectedCollection = (period: string) =>
  createSelector(
    fromContributionTypes.selectRepeating,
    fromLoan.selectAll,
    fromMember.selectAll,
    (contributionTypes, loans, members) => {
      return members.map((member) => {
        const activeLoans = loans.filter((i) => {
          return (
            `${i.end_year}${i.end_month}` >= period && i.member_id === member.id
          );
        });
        let totalLoan = 0;
        let totalExpected = 0;
...
export const selectContributionAndLoanForSingleMonth = (period: string) =>
  createSelector(
    selectDetailed,
    fromMember.selectAll,
    fromLoan.selectAll,
...
export const selectContributionByMonth = (month, year) =>
  createSelector(selectDetailed, (allItems) => {
    const items = allItems.filter(
      (i) =>
        month === i.month &&
        i.contributionsDetails.length > 0 &&
        year + '' === i.year + ''
    );
    return items;
  });

export const selectContributionByMonthByMember = (month, year, memberId) =>
  createSelector(selectContributionByMonth(month, year), (contributions) => {
    const contri = contributions.filter(
      (contribution) => contribution.memberId === memberId
    );
    return contri;
  });

export const selectTotalPaymentByYear = (year, contributionType, memberId?) =>
  createSelector(selectDetailed, (allItems) => {
    const items = allItems
      .filter((i) => year === 'All' || i.year + '' === year + '')
      .filter(
        (i) =>
          !!i.contributionsDetails.find(
            (k) => k.contr_id === contributionType || contributionType === 'All'
          )
      )
      .filter((i) => memberId === 'All' || i.memberId === memberId);
    // const items = allItems;
...
export const selectGroupTotalPayments = createSelector(
  ...
  (allItems) => {
    ...
    let sum = 0;
    for (const item of items) {
      const contr = Object.keys(item.contributions).map(
        (i) => item.contributions[i]
      );
      for (const amount of contr) {
        sum += !!(amount + '') ? parseFloat(amount + '') : 0;
      }
...
export const selectTotalLoanPaymentByYear = (
  year,
  contributionType,
  memberId?
) =>
    ...
    (allItems, loanTypes, loans) => {
      ...
      for (const item of items) {
        const contr = Object.keys(item.loans)
          .map((i) => ({
            loanUsed: loans[i] ? loanTypes[loans[i].loan_used] : null,
            amount: item.loans[i],
          }))
          .filter(
            (i) =>
              (!!i.loanUsed && contributionType === 'All') ||
              (i.loanUsed &&
                i.loanUsed.contribution_type_id === contributionType)
          );
        for (const amount of contr) {
...
export const selectTotalFinePaymentByYear = (
  year,
  contributionType,
  memberId?
) =>
        ...
        const contr = Object.keys(item.fines)
          ...
          .filter(
            (i) =>
              ...
              (i.fineType &&
                i.fineType.contribution_type_id === contributionType)
          );
        for (const amount of contr) {
          sum += !!(amount.amount + '') ? parseFloat(amount.amount + '') : 0;
        }
...
export const selectTotalIn = (year, contributionType, memberId?) =>
  createSelector(
...
export const selectTotalContributions = (year, contributionType, memberId) =>
  createSelector(
    selectTotalPaymentByYear(year, contributionType, memberId),
    selectTotalLoanPaymentByYear(year, contributionType, memberId),
    selectTotalFinePaymentByYear(year, contributionType, memberId),
...
export const selectTotalByYear = (year, contributionType, memberId?) =>
  createSelector(
...
export const selectFinesByMemberByYear = (year, memberId, fineType) =>
  createSelector(selectDetailed, (allItems) =>
    allItems
      .filter((i) => i.memberId === memberId)
      .filter((i) => year === 'All' || i.year + '' === year + '')
      .filter(
        (i) =>
          !!i.fineDetails.find((k) => k.id === fineType || fineType === 'All')
      )
      .map((i) => ({
        ...i,
        fineKeys: fineType === 'All' ? i.fineKeys : [fineType],
        totalFines: i.fineDetails
          .filter((k) => fineType === 'All' || k.id === fineType)
          .map((k) => k.amount)
          .reduce((j, k) => k + j),
        description: i.fineDetails
          .filter((k) => fineType === 'All' || k.id === fineType)
          .map((k) => `${k.name} ${numberWithCommas(k.amount)}`)
          .join(', '),
      }))
...
export const selectFinesByMonthByYear = (year, month, fineType) =>
  createSelector(selectDetailed, (allItems) =>
    allItems
      .filter(
        (item) => item.month + '' === month + '' && item.year + '' === year + ''
