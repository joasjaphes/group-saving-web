...
export const selectCurrentState = createFeatureSelector<fromReducer.State>(fromReducer.fineTypesFeatureKey);
...
export const selectByCurrentGroup = createSelector(selectAll, selectGroupId, (allItems, groupId) => allItems.filter((i) => i.group_id === groupId));
...
export const selectMeetingNotAttendingFineType = createSelector(selectDetailed, (items) => items.find((item) => item.type == "Meeting" && item.meeting_type == "not_attending"));

export const selectLateContributionFineType = (contributionType: string) => createSelector(selectDetailed, (items) => items.find((item) => item.type == "Contribution" && item.contribution_type_id == contributionType));
export const selectLateReturnsFine = (loanType: string) => createSelector(selectDetailed, (items) => items.find((item) => item.type == "Loan" && item.loan_type_id === loanType));
