...
export class RegistrationComponent implements OnInit {
  ...
  constructor(
    ...
    this.phoneCountry$ = this.store.pipe(
      select(loginSelector.selectPhoneCountry)
    );
    this.currentStep$ = this.store.pipe(
      select(loginSelector.selectCurrentStep)
    );
    this.previousStep$ = this.store.pipe(
      select(loginSelector.selectPreviousStep)
    );
    this.language$ = this.store.pipe(select(loginSelector.selectLanguage));
    this.phoneNumber$ = this.store.pipe(
    ...
    this.progressValue$ = this.store.pipe(
      select(loginSelector.selectProgressValue)
    );
    this.firstPassword$ = this.store.pipe(
      select(loginSelector.selectFirstPassword)
    );
    this.secondPassword$ = this.store.pipe(
      select(loginSelector.selectSecondPassword)
    );
    ...
    this.memberGroups$ = this.store.pipe(
      select(loginSelector.selectMemberGroups)
    );
    this.phoneNumberValid$ = this.store.pipe(
      select(loginSelector.phoneNumberValid)
    );
  }
  ...
  async save() {
    const data = await this.store
      .pipe(select(selectObjectToSave))
      .pipe(first())
      .toPromise();
    const memberGroups = await this.memberGroups$.pipe(first()).toPromise();
    const url =
      memberGroups && memberGroups.length > 0 ? 'createUserOnly' : 'createUser';
