 ...
 */
export const updatePhoneNumber = functions.https.onRequest((request, response) => {
        ...
        if (groupMemberRef && groupMemberRef.docs) {
          groupMemberRef.docs.forEach(doc => {
            if (doc.data()) {
              ...
              transaction.update(groupMemberDocRef, {last_update, phone_number: data.updated_phone});
            }
          });
        }
