 ...
 */
export const removeMemberFromLoanQueue = functions.https.onRequest((request, response) => {
  return cors(request, response, async () => {
    ...
    try {
      ...
      await admin.firestore().runTransaction(async (transaction) => {
        ...
        const loan_queue = old_loan_queue.filter((item: any) => !(item.id === loanQueueId));
      ...
      console.log('Error removing member from to loan queue:', e);
